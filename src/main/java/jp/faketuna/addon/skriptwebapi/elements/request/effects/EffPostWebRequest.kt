...
@Examples("set {_header} to blank header\n" +
        ...
        "send post request to \"http://domain/\" with header {_header} and body \"{\"\"json\"\":\"\"body\"\"}\"\n")
@Since("0.0.3")
class EffPostWebRequest: Effect() {

    companion object{
        init {
            Skript.registerEffect(EffPostWebRequest::class.java,
    ...
    override fun execute(e: Event) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
            ...
            Bukkit.getScheduler().callSyncMethod(plugin){
                if (response != null) {
                    Bukkit.getPluginManager().callEvent(PostResponseEvent(response))
                } else{
