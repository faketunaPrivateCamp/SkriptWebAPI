...
@Examples("set {_header} to blank header\n" +
        ...
        "set {_response} to response of post request to \"http://domain/\" with header {_header} and body \"{\"\"json\"\":\"\"body\"\"}\"\n" +
        "broadcast \"response: %{_response}'s response body%\"")
@Since("0.0.2")
class ExprWebRequestPost: SimpleExpression<HttpURLConnection>() {

    companion object{
        init {
            Skript.registerExpression(ExprWebRequestPost::class.java,
                HttpURLConnection::class.java,
                ExpressionType.COMBINED,
                "[skeb] response of post request to [url] %string% [(with|and)] header %header% [(with|and)] body %string%"
            )
        }
    }
