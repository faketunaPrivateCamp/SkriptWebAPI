...
@Name("Send a async put web request")
@Description("Send a async put web request with specified header and body.\n" +
        "It calls put response event when connection successful.")
@Examples("set {_header} to blank header\n" +
        ...
        "send put request to \"http://domain/\" with header {_header} and body \"{\"\"json\"\":\"\"body\"\"}\"\n")
@Since("0.0.3")
class EffPutWebRequest: Effect() {

    companion object{
        init {
            Skript.registerEffect(EffPutWebRequest::class.java,
    ...
    override fun execute(e: Event) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, Runnable {
            ...
            Bukkit.getScheduler().callSyncMethod(plugin){
                if (response != null) {
                    Bukkit.getPluginManager().callEvent(PutResponseEvent(response))
                } else{
